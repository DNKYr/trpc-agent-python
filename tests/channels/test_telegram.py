...
def test_verify_signature_ok():
    ...
    assert adapter.verify_signature(token="", secret="s3cr3t", signature="s3cr3t", timestamp="", nonce="",
                                    payload="") is True


def test_verify_signature_bad():
    adapter = TelegramAdapter()
    assert adapter.verify_signature(token="", secret="s3cr3t", signature="nope", timestamp="", nonce="",
                                    payload="") is False


def test_verify_signature_empty_secret():
    adapter = TelegramAdapter()
    assert adapter.verify_signature(token="", secret="", signature="", timestamp="", nonce="", payload="") is False
